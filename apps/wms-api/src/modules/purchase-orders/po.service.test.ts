  ...
  it('creates new PO and lines when sap_po_number is new', async () => {
    const dbRead = {
      query: vi
        ...
        // SKU 1 is Active
        .mockResolvedValueOnce({ rows: [{ status: 'Active' }], rowCount: 1 } as any as QueryResult)
        // SKU 2 is Active
        .mockResolvedValueOnce({ rows: [{ status: 'Active' }], rowCount: 1 } as any as QueryResult),
    } as unknown as Pool;
    ...
    const db = {
      query: vi
        ...
        .mockResolvedValueOnce({ rows: [MOCK_PO], rowCount: 1 } as any as QueryResult)
  ...
  it('flags PO line as Blocked when SKU is not Active', async () => {
    const dbRead = {
      query: vi
        ...
        .mockResolvedValueOnce({ rows: [{ status: 'Inactive' }], rowCount: 1 } as any as QueryResult)
        // SKU 2 is Active
        .mockResolvedValueOnce({ rows: [{ status: 'Active' }], rowCount: 1 } as any as QueryResult),
    } as unknown as Pool;
  ...
  it('closes line without backorder when short-delivered (BR-13)', async () => {
    ...
    const dbRead = {
      query: vi
        .fn()
        // Fetch line
        .mockResolvedValueOnce({ rows: [MOCK_LINE_1], rowCount: 1 } as any as QueryResult)
        // Remaining open lines count (0 remaining)
        .mockResolvedValueOnce({ rows: [{ count: '0' }], rowCount: 1 } as any as QueryResult),
    } as unknown as Pool;
    ...
    const db = {
      query: vi
        .fn()
        // UPDATE po_lines
        .mockResolvedValueOnce({ rows: [closedLine], rowCount: 1 } as any as QueryResult)
  ...
  it('transitions PO to PartiallyClosed when some lines remain open', async () => {
    ...
    const dbRead = {
      query: vi
        ...
        .mockResolvedValueOnce({ rows: [{ count: '1' }], rowCount: 1 } as any as QueryResult),
    } as unknown as Pool;
  ...
  it('returns PO with all lines and current status', async () => {
    const dbRead = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rows: [MOCK_PO], rowCount: 1 } as any as QueryResult)
        .mockResolvedValueOnce({ rows: [MOCK_LINE_1, MOCK_LINE_2], rowCount: 2 } as any as QueryResult),
    } as unknown as Pool;
